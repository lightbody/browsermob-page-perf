...
public class ObjectResponseTimeMetric implements Metric<ObjectResponseTimeEntry> {
    ObjectResponseTimeMetric() {
    }
    ...
    @Override
    public ObjectResponseTimeEntry parse(ResultSet rs, Rollup rollup, TimeZone timeZone) throws SQLException {
        Long responseTime = rs.getLong("time_active");

        ObjectResponseTimeEntry r;
        ...
        if (rollup == Rollup.NONE) {
            ...
            r = new ObjectResponseTimeEntry(targetCal.getTime(), responseTime);
            ...
            r = new ObjectResponseTimeEntry(cal.getTime(), responseTime);
